...
func (s *GrpcServer) Start(cfg *config.Config) error {
	...
	r := repo2.NewRepo(s.db, s.batchSize)
	er := repo2.NewEventRepo(s.db, s.batchSize)
	notificationRepo := repo2.NewNotificationRepo(s.db, s.batchSize)

	pb.RegisterActDeviceApiServiceServer(grpcServer, api.NewDeviceAPI(r, er))
	pb.RegisterActNotificationApiServiceServer(grpcServer, api.NewNotificationAPI(notificationRepo))
